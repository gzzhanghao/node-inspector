...
class Backend extends EventEmitter {
  ...
  constructor(connection, config) {
    ...
    for (const plugin of config.plugins) {
      this.ready = this.ready.then(() => plugin(this));
    }
    this.ready.catch(error => this.emit('error', error));
  }
  ...
  eval(expression, options) {
    return this.request('evaluate', Object.assign({
      expression,
      global: true
    }, options));
  }
  ...
  close() {
    ...
    return co(function * () {
      ...
      yield Promise.race([
        new Promise(resolve => this._conn.once('close', resolve)),
        new Promise(resolve => setTimeout(resolve, 200))
      ]);

      if (!closed) this._conn.destroy();
      this._conn = null;
    }.bind(this));
  }
