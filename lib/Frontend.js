...
class Frontend extends EventEmitter {
  ...
  constructor(conn, backend, config) {
    ...
    backend.connected.then(this._notifyBackendConnected.bind(this));
    backend.ready.then(this._notifyBackendReady.bind(this));

    backend.on('unhndledMessage', this._notifyBackendUnhandled.bind(this));
    backend.on('event', this._notifyBackendEvent.bind(this));
    backend.on('error', this._notifyBackendError.bind(this));
    backend.once('close', this._handleBackendClose.bind(this));
  }

  close(reason) {
    if (!this.conn) return Promise.resolve();
    return co(function * () {
      yield this._send({ method: 'Inspector.detached', params: { reason } }).catch(() => {});
      this.conn.close();
    }.bind(this));
  }
  ...
  _notifyBackendConnected() {
    this._sendLog('log', 'Backend connected');
  }

  _notifyBackendReady() {
    this._sendLog('log', 'Backend ready');
  }

  _notifyBackendUnhandled(msg) {
    this._sendLog('warning', 'Unhandled message', msg);
  }
