...
class Frontend extends EventEmitter {
  ...
  constructor(conn, backend, config) {
    ...
    backend.ready
      .catch(this._notifyPluginError.bind(this))
      .then(this._notifyBackendMessage.bind(this));
  ...
  _handleConnMsg(data) {
    co(function * () {
      ...
      try {
        ...
        this._sendLog('error', msg.method, error.stack || error);
      }
  ...
  _handleConnError(error) {
    this._sendLog('error', 'NodeInspector Connection', error.stack || error);
  }
  ...
  _notifyPluginError(error) {
    this._sendLog('error', 'NodeInspector Plugin', error.stack || error);
  }

  /**
   * Send backend existing messages
   *
   * @private
   */
  _notifyBackendMessage() {
  ...
  _notifyBackendError(error) {
    this._sendLog('error', 'NodeInspector Backend', error.stack || error);
  }
