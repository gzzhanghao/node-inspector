...
function list(root, include, exclude) {
  include = include || [];
  exclude = exclude || [];
  var relative = path.relative.bind(path, root);
  var folders = [root];
  var cache = {};
  var list = [];
  ...
  function _list(node) {
    return co(function * () {
      let paths;
      try {
        paths = yield readdir(node);
      } catch (e) {
        debug(`Access to ${node} from ResourceTree denied. (${e})`);
        return;
      }

      yield paths.map(child => co(function * () {
        child = path.join(node, child);
        const rpath = relative(child);

        if (cache[child] || exclude.some(rx => rx.test(rpath))) return;
        cache[child] = true;

        try {
          const stat = yield stat(child);
          if (stat.isDirectory()) return folders.push(child);
          if (stat.isFile() && include.some(rx => rx.test(rpath))) list.push(child);
        } catch (e) {
          debug(`Access to ${child} from ResourceTree denied. (${e})`);
        }
      }));
    });
  }
