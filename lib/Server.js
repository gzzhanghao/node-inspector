...
class Server extends EventEmitter {
  ...
  _createSession(conn) {
    ...
    if (!this.backends[port]) {
      const backend = new Backend(net.createConnection(port), this.config);
      this.backends[port] = backend;
      this.refCount[port] = 0;
    ...
    this.refCount[port] += 1;
    frontend.once('close', () => {
      this.refCount[port] -= 1;
      if (!this.refCount[port] && this.backends[port]) this.backends[port].close();
    });
    this.emit('frontend', frontend);
  }
  ...
  _handleIndexRequest(req, res) {
    if (!req.query.ws) {
      return res.redirect(`${req.originalUrl}?ws=${this.config.host}:${this.config.port}`);
    }
