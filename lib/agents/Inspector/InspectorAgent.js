...
module.exports = {
  ...
  inject: co.wrap(function * (backend) {
    ...
    const heartbeatInterval = backend.config.inspectorHeartbeatInterval || 60 * 1000;

    setInterval(() => {
      backend.request('NodeInspector.heartbeat');
    }, heartbeatInterval);
    ...
    const options = JSON.stringify({ heartbeatInterval });

    // Inject injector server into target process
    const result = yield evaluate(
      `NM.require('module')._load(${serverPath}).inject(${options})`,
