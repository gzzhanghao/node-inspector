...
module.exports = function(session) {
  ...
  const helper = session.backend.scripts = {
    ...
    addScript(script) {
      ...
      session.send('Debugger.scriptParsed', scriptData);
    },
    ...
    resync() {
      return co(function * () {
        ...
        const scripts = yield session.request('scripts', {
          types: 4,
          includeSource: true
        });
    ...
    setScriptSource(scriptId, newSource, previewOnly) {
      return co(function * () {
        ...
        const response = yield session.request('changeLive', {
          script_id: scriptId,
