    ...
    const helper = {
      ...
      getScriptSource(id) {
        return co(function * () {
          ...
          if (!scripts.length) {
            console.log(this.resources.filter(s => !bugs.contains(s.scriptId)))
            const script = yield this.getScriptById(id);
            return yield fs.readFile(script.name);
          }
      ...
      setScriptSource(id, source, preview) {
        return co(function * () {
          ...
          if (backend.config.saveLiveEdit && !preview) {
            ...
            yield fs.writeFile(script.name, source.slice(scriptPrefix.length, -scriptPostfix.length));
          }
