...
module.exports = function(backend) {
  ...
  const scripts = backend.scripts = {
     ...
     */
    add(v8Script) {
      if (this.resources.find(s => s.id == v8Script.id)) return;

      const name = scripts.scriptName(v8Script.name);
      const internal = scripts.scriptIsInternal(name);

      const script = {
        name, internal,
        scriptId: String(v8Script.id),
        url: scripts.path2url(name),
    ...
    getScriptById(id) {
      return co(function * () {
        const script = this.resources.find(s => s.id == id);
        if (script) return script;
        
        const scripts = yield backend.request('scripts', { filter: id });
        if (!scripts.length) throw new Error(`Cannot get script with id ${id}`);
    ...
    getScriptSource(id) {
      return co(function * () {
        const scripts = yield backend.request('scripts', { includeSource: true, filter: id });
        if (!scripts.length) throw new Error(`Cannot get source of script ${id}`);
    ...
    setScriptSource(id, source, preview) {
      return co(function * () {
        if (!source.startsWith(scriptPrefix) || !source.endsWith(scriptPostfix)) {
        ...
        if (backend.config.saveLiveEdit && !preview) {
          ...
          if (script.internal) throw new Error('Cannot set content of internal script');
          yield writeFile(script.name, source.slice(scriptPrefix.length, -scriptPostfix.length));
        }
